...
def prepare_header(seq):
    ...
    for parameter in header:
        try:
            tempstr, tempbyte = HexToBytes(parameter['value'], int(parameter['size']), 'little', signed)
        except Exception as ex:
...
# COUNT_COLUMN_INDEX:{COUNT_COLUMN_INDEX}
# TYPE_COLUMN_INDEX:{TYPE_COLUMN_INDEX}
# LENGTH_COLUMN_INDEX:{LENGTH_COLUMN_INDEX}
# SIMULATE_COLUMN_INDEX:{SIMULATE_COLUMN_INDEX}
# MAX_COLUMN_INDEX:{MAX_COLUMN_INDEX}
# MIN_COLUMN_INDEX:{MIN_COLUMN_INDEX}
# INC_COLUMN_INDEX:{COUNT_COLUMN_INDEX}
# INC_START_COLUMN_INDEX:{INC_START_COLUMN_INDEX}
# INC_END_COLUMN_INDEX:{INC_END_COLUMN_INDEX}
# INC_STEP_COLUMN_INDEX:{INC_STEP_COLUMN_INDEX}
# TIMESEG_COLUMN_INDEX:{TIMESEG_COLUMN_INDEX}
# FORMULA_COLUMN_INDEX:{FORMULA_COLUMN_INDEX}
# DATASTART_COLUMN_INDEX:{DATASTART_COLUMN_INDEX}
#           ''')
...
def prepare_telegram_list():
    ...
    for telegram in telegram_list:
        if telegram['@type'] !='HeaderType':
            print(f"{telegram['@name']} {telegram['@count']} {telegram['@type']} {telegram['@length']}")
...
def prepare_telegram():
    ...
    for telegram in telegram_list:
        if telegram['@type'] !='HeaderType':
            ...
            dim = list()
            loop = list()
            if ';' not in telegram['@count']:
                dim.append(int(telegram['@count']))
                loop.append(1)
            else:
                dimensions = telegram['@count'].split(';')
                for i in range(len(dimensions)):
                    dim.append(int(dimensions[i]))
                    loop.append(1)
            ...
            while loop[0] <= dim[0]:
                if len(dim) == 1 and dim[0]==1:
                    ...
                    print(f'{name}_{loop[0]}')
                elif len(dim) == 2:
                    while loop[1] <= dim[1]:
                        if dim[0]==1:
                            print(f'{name}_{loop[1]}')
                        elif dim[1]==1:
                            print(f'{name}_{loop[0]}')
                        else:
                            print(f'{name}_{loop[0]}_{loop[1]}')
                        loop[1] += 1
                loop[0] += 1

def csv_read():
    with open(r'Telegram_List.csv', mode ='r')as file:
...
def prepare_telegram_data():
    data = csv_read()
    seq = 1
    headerstr, headerbyte = prepare_header(seq)
    bodystr, bodybyte = prepare_body(data)
    #print(f"{headerstr}\n{headerbyte}")
    #print(f"\n----------------------------------\n")
    #print(f"{bodystr}\n{bodybyte}")
